...
###############################################################################  
 ...
 # are for you. Theyâ€™ll tell you what the code is doing at all times. The nonâ€“hash-tag
 # lines are used by the computer, though if you spend a few seconds with them, 
 # youâ€™ll be able to understand exactly what theyâ€™re doing.

 # Since you opened this, you probably already have a good idea of what UTXOracle
 # does. It finds the price of Bitcoin using data only from your own node. It doesnâ€™t
 ...
 # have Python 3 built in, but many donâ€™t. Youâ€™ll also need to know how to open a
 # terminal window on your computer. To see if you already have Python 3 installed,
 # open a terminal window and type python3 --version, then hit Enter. If it doesnâ€™t
 # show you a version number, youâ€™ll need to download and install it from python.org.

 # If youâ€™re reading this from a file on your computer, then you already have the
 # program. If youâ€™re reading this off a website, you need to download it to your
 ...
 # If youâ€™re an experienced coder, youâ€™ll notice many inefficiencies in this code.
 # It is a single file that runs top to bottom. 
 # It doesnâ€™t not make use of advanced libraries that would make the code more
...
# does not contain enough transaction data to determine the priceâ€”and in some
...
# the nodeâ€™s block data, and (2) the date the user is requesting the price for. If the 
...
# Bitcoin nodes donâ€™t store blocks by dateâ€”or even by block height. Instead, they 
...
# Once weâ€™ve identified all the relevant block heights, we store the hashes of each 
...
# cluster. If the intervals are too small, the data becomes noisy; if theyâ€™re too large, 
# important detail is lost. A rough estimate of the ideal interval width is the average 
# daily fiat volatility, which weâ€™ve found to be around 0.5%â€”corresponding to roughly 
...
# evolve as Bitcoinâ€™s purchasing power changes and may need to be updated in 
# future versions of UTXOracle. From 2020 to 2025, weâ€™ve found that most round fiat 
...
# Users donâ€™t just send round fiat amounts, they also tend to send them in 
...
# Through historical testing, weâ€™ve measured the average histogram bin counts for 
...
# behavior is still influenced by Bitcoinâ€™s long-term rise in purchasing power. For 
...
# UTXOracle as Bitcoinâ€™s purchasing power changes. We also assign a relative 
...
# in the userâ€™s default web browser using the webbrowser.open() command.
